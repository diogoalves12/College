...
Point& Point::operator=(const Point& p) { x_ = p.x_; y_ = p.y_; return *this;}
...
Point operator*(int x, const Point& p) { return Point(p.get_x() * x,p.get_y()*x);}
std::ostream& operator<<(std::ostream& os, const Point& p){
    os << '(' << p.get_x() << ',' << p.get_y() << ')'; return os;}